...
class APITests(unittest.TestCase):
    ...
    def test_query(self):
        response = self.client.get('/api/support-groups?query=Bay')
        assert response
        self.assertEqual(response.status_code, 200)

    def test_news_q(self):
        response = self.client.get('/api/countries?query=Palestine')
        assert response
        self.assertEqual(response.status_code, 200)

    def test_countries_query(self):
        response = self.client.get('api/news?query=Palestine')
        assert response
        self.assertEqual(response.status_code, 200)
